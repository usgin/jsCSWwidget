...
usgin.CSWwidget.prototype.init = function() {
	...
	frm.append($("<tr>")
		...
		.append($("<td class='hint' colspan=3>").text("Hint: Use AND or OR between multiple search terms"))
	).append($("<tr>")
...
usgin.CSWwidget.prototype.getWmsCheck = function() {
	if (this.wmsCheck == null) {
		var wc = $($.parseHTML("<input type='checkbox' class='csw-check' id='wmsCheck' checked disabled>"));
		this.wmsCheck = wc;
	}
...
usgin.CSWwidget.prototype.getWfsCheck = function() {
	if (this.wfsCheck == null) {
		var wc = $($.parseHTML("<input type='checkbox' class='csw-check' id='wfsCheck' checked>"));
		this.wfsCheck = wc;
	}
...
usgin.CSWwidget.prototype.getLaunchBtn = function() {
	if (this.launchBtn == null) {
		var btn = $($.parseHTML("<input type='button' value='Add To Map' class='csw-infoboard-add-btn csw-infoboard-btn-disabled' />"));
		//var btn = $($.parseHTML("<i class='fa fa-picture-o fa-fw csw-infoboard-btn csw-infoboard-btn-disabled' title='add layer'/>"));
		var w = this;
		btn.click(function(e) {
...
usgin.CSWwidget.prototype.findPage = function(startPosition) {
	...
	var strKey = this.getFindInput().val();
	//strKey += this.getWmsCheck().prop('checked') ? " wms" : "";
	//strKey += this.getWfsCheck().prop('checked') ? " wfs" : "";

	var keys = strKey.split(/,|\s+|;/), key="";
	...
	csw.init(function(xml){
		w.loading();
		csw.getRecords([strKey], w.getFieldSelector().val(), w.getBBoxFilter(),
			w.getWmsCheck().prop('checked'),
			w.getWfsCheck().prop('checked'),
			//false, false,
			startPosition, function(results) {
			w.results = results;
			w.displayResults();
		}, function(msg) {
			w.logger.error(msg);
