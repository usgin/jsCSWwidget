...
usgin.CSW.prototype.getRecords = function(literal, property, bbox, wms_filter, wfs_filter, startPosition, success, error) {
	...
	if (literal != null && literal.length > 0){
        ...
        params.Constraint = this.buildFilter(literal, property, bbox, wms_filter, wfs_filter);
	}
	var w = this;
...
usgin.CSW.prototype.buildFilter = function(literal, property, bbox, wms_filter, wfs_filter) {
	var b = usgin.Util.validVar(bbox) || wms_filter || wfs_filter || literal.length > 1;
	var c = "<ogc:Filter xmlns:ogc='http://www.opengis.net/ogc' xmlns:gml='http://www.opengis.net/gml'>";
	c += b ? "<ogc:And>" : "";
	c += wms_filter ? "<ogc:PropertyIsLike wildCard='%' singleChar='_' escape='\\'>"
		+ "<ogc:PropertyName>subject</ogc:PropertyName>"
		+ "<ogc:Literal>%wms%</ogc:Literal></ogc:PropertyIsLike>" : "";
	c += wfs_filter ? "<ogc:PropertyIsLike wildCard='%' singleChar='_' escape='\\'>"
		+ "<ogc:PropertyName>subject</ogc:PropertyName>"
	...
	for (var i=0; i<literal.length; i++) {
		c += "<ogc:PropertyIsLike wildCard='%' singleChar='_' escape='\\'>"
			...
			+ "<ogc:Literal>" + literal[i] + "</ogc:Literal></ogc:PropertyIsLike>";
	}
